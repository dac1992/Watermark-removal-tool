...
export const ImageCanvas: React.FC<ImageCanvasProps> = ({ 
  imageUrl, boxes, setBoxes, lines, setLines, onClear, drawMode, setDrawMode, brushSize, onSizeChange 
}) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (imageRef.current) {
        ...
        if (onSizeChange) {
           onSizeChange({ canvasWidth: clientWidth, canvasHeight: clientHeight, videoWidth: naturalWidth || clientWidth, videoHeight: naturalHeight || clientHeight });
        }
      }
    };
    window.addEventListener('resize', handleResize);
    
    // image load event will also fire handleResize
    
    return () => window.removeEventListener('resize', handleResize);
  }, [imageUrl, onSizeChange]);
